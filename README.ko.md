...
- 영어 로그인 화면에서 저장된 NAS 프로필의 이름, 상태 배지, URL이 겹치거나 잘리지 않도록 레이아웃을 다듬었습니다.
- 설정, 서버 편집, 대기열 제어, 플레이리스트, 로딩/재시도 상태, 언어/음질 선택 주변의 누락된 번역을 보완했습니다.
- 강조색 즉시 반영, QuickConnect 호환성, 셔플 중 선택 재생, 앨범 그리드, 로딩 토스트, 플레이어 메뉴와 대기열 편집 동작을 수정했습니다.
