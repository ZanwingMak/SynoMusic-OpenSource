...
[公式サイト](https://zanwingmak.github.io/SynoMusic-OpenSource/) · [最新リリース](https://github.com/ZanwingMak/SynoMusic-OpenSource/releases/latest)
...
このリポジトリには、SynoMusic iOS App のオープンソース版が含まれています。メイン App、Live Activity 拡張、ローカルプレイリストとダウンロード保存、Synology Audio Station API クライアント、GitHub Pages サイトを含みます。

- 利用には自分の Synology NAS、DSM アカウント、Audio Station 権限が必要です。デモサーバー、プロキシ、組み込み認証情報は含まれていません。
- Release IPA は利便性のために提供されています。ソースからビルドする場合は、自分の Apple 署名チームを Xcode で設定してください。
- 認証情報は iOS Keychain に保存され、ダウンロードした曲は App サンドボックス内に保存されます。
- スクリーンショットと例は匿名化されたサンプルデータを使用しています。実際の NAS ホスト、QuickConnect ID、アカウント情報をコミットしないでください。
...
- 他の App の音声割り込み後も再生ボタンの状態が正しく同期されるよう修正しました。
- 割り込み終了後、SynoMusic が割り込み前の状態に応じて自動再開するよう修正しました。
...
Xcode プロジェクトは [`project.yml`](project.yml) から [XcodeGen](https://github.com/yonaskolb/XcodeGen) で生成されています。プロジェクト構成、Target、ビルド設定、リソースを変更した場合は再生成してください。
...
Xcode → Signing & Capabilities で `SynoMusic` と `SynoMusicLive` に自分の開発者署名を設定してから、シミュレータまたは実機で実行してください。
